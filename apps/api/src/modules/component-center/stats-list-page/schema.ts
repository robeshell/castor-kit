/**
 * 带统计的列表页 schema 层（对齐 AuraStack backend/app/component_center/schema/stats_list_page.py）
 */
...
/** Python float → SQLAlchemy Numeric 绑定参数文本（psycopg2 用 repr(float)） */
...
/**
 * Python `float == Decimal`（SQLAlchemy 判断属性是否变更用）：精确比较。
...
/** Python `round(x, 2)`（浮点、银行家舍入只在二进制精确的 .xx5 上生效） */
