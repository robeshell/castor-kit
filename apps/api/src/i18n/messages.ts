...
export const MESSAGES: Record<string, MessageEntry> = {
  ...
  '在线建模只对超级管理员开放': { 'en-US': 'The visual modeler is for super admins only', 'ja-JP': 'オンラインモデリングはスーパー管理者のみ使用できます' },
  '已有建模任务在运行，请等它结束': { 'en-US': 'A modeler job is already running. Please wait for it to finish.', 'ja-JP': 'モデリングのジョブが実行中です。終了するまでお待ちください。' },
  '没有找到这个模块，只能撤销建模器生成的模块': { 'en-US': 'Module not found. Only modules generated by the modeler can be undone.', 'ja-JP': 'モジュールが見つかりません。取り消せるのはモデラーで生成したモジュールだけです。' },
  '任务不存在': { 'en-US': 'Job not found', 'ja-JP': 'ジョブが見つかりません' },
  '请描述要做的功能': { 'en-US': 'Describe the feature you want', 'ja-JP': '作りたい機能を説明してください' },
  '描述最多 1000 个字符': { 'en-US': 'The description can be at most 1000 characters', 'ja-JP': '説明は 1000 文字以内で入力してください' },
  'AI 生成失败，请稍后重试或手动填写': { 'en-US': 'AI generation failed. Try again later or fill it in by hand.', 'ja-JP': 'AI による生成に失敗しました。しばらくしてから再度お試しいただくか、手動で入力してください。' },
  'spec 必须是 JSON 对象': { 'en-US': 'The spec must be a JSON object', 'ja-JP': 'spec は JSON オブジェクトである必要があります' },
  '模块名必须是 snake_case（小写字母开头，只含小写字母、数字、下划线，最多 40 个字符）': { 'en-US': 'The module name must be snake_case (starts with a lowercase letter; lowercase letters, digits and underscores; at most 40 characters)', 'ja-JP': 'モジュール名は snake_case にしてください（小文字で始まり、小文字・数字・アンダースコアのみ、40 文字以内）' },
  'domain 只能是 admin 或 component_center': { 'en-US': 'domain must be admin or component_center', 'ja-JP': 'domain は admin または component_center のみです' },
  '标题不能为空，最多 50 个字符': { 'en-US': 'The title is required, at most 50 characters', 'ja-JP': 'タイトルは必須で、50 文字以内です' },
  '标题不能包含引号、反斜杠、花括号、尖括号或换行': { 'en-US': "The title can't contain quotes, backslashes, braces, angle brackets or line breaks", 'ja-JP': 'タイトルに引用符・バックスラッシュ・波かっこ・山かっこ・改行は使えません' },
  '至少需要一个字段': { 'en-US': 'Add at least one field', 'ja-JP': 'フィールドを 1 つ以上追加してください' },
  '字段最多 50 个': { 'en-US': 'At most 50 fields', 'ja-JP': 'フィールドは最大 50 個です' },
  '父菜单 ID 不正确': { 'en-US': 'Invalid parent menu ID', 'ja-JP': '親メニュー ID が正しくありません' },
...
export const PATTERNS: Array<{ re: RegExp } & MessageEntry> = [
  ...
  { re: /^字段 (.+)：字段名必须是 snake_case，最多 40 个字符$/, 'en-US': 'Field $1: the name must be snake_case, at most 40 characters', 'ja-JP': 'フィールド $1：名前は snake_case、40 文字以内にしてください' },
  { re: /^字段 (.+)：(.+) 是保留字段名$/, 'en-US': 'Field $1: $2 is a reserved name', 'ja-JP': 'フィールド $1：$2 は予約されたフィールド名です' },
  { re: /^字段 (.+)：字段名重复$/, 'en-US': 'Field $1: duplicate name', 'ja-JP': 'フィールド $1：名前が重複しています' },
  { re: /^字段 (.+)：未知类型 (.+)$/, 'en-US': 'Field $1: unknown type $2', 'ja-JP': 'フィールド $1：不明な型 $2' },
  { re: /^字段 (.+)：标签最多 50 个字符$/, 'en-US': 'Field $1: the label can be at most 50 characters', 'ja-JP': 'フィールド $1：ラベルは 50 文字以内です' },
  { re: /^字段 (.+)：文件 \/ 图片字段不能设为必填$/, 'en-US': 'Field $1: file / image fields can\'t be required', 'ja-JP': 'フィールド $1：ファイル / 画像フィールドは必須にできません' },
  { re: /^字段 (.+)：只有文本和数字字段可以设为唯一$/, 'en-US': 'Field $1: only text and number fields can be unique', 'ja-JP': 'フィールド $1：一意にできるのはテキストと数値のフィールドだけです' },
  { re: /^字段 (.+)：固定选项至少要有一项$/, 'en-US': 'Field $1: add at least one option', 'ja-JP': 'フィールド $1：選択肢を 1 つ以上追加してください' },
  { re: /^字段 (.+)：选项值只能包含字母、数字、下划线和连字符（最多 50 个字符）$/, 'en-US': 'Field $1: option values may only contain letters, digits, underscores and hyphens (at most 50 characters)', 'ja-JP': 'フィールド $1：選択肢の値に使えるのは英数字・アンダースコア・ハイフンのみです（50 文字以内）' },
  { re: /^字段 (.+)：选项值 (.+) 重复$/, 'en-US': 'Field $1: option value $2 is duplicated', 'ja-JP': 'フィールド $1：選択肢の値 $2 が重複しています' },
  { re: /^字段 (.+)：选项名称不能为空，最多 50 个字符$/, 'en-US': 'Field $1: option names are required, at most 50 characters', 'ja-JP': 'フィールド $1：選択肢の名前は必須で、50 文字以内です' },
  { re: /^字段 (.+)：标签不能包含引号、反斜杠、花括号、尖括号或换行$/, 'en-US': "Field $1: the label can't contain quotes, backslashes, braces, angle brackets or line breaks", 'ja-JP': 'フィールド $1：ラベルに引用符・バックスラッシュ・波かっこ・山かっこ・改行は使えません' },
  { re: /^字段 (.+)：选项名称不能包含引号、反斜杠、花括号、尖括号或换行$/, 'en-US': "Field $1: option names can't contain quotes, backslashes, braces, angle brackets or line breaks", 'ja-JP': 'フィールド $1：選択肢の名前に引用符・バックスラッシュ・波かっこ・山かっこ・改行は使えません' },
  { re: /^字段 (.+)：请选择字典$/, 'en-US': 'Field $1: choose a dictionary', 'ja-JP': 'フィールド $1：辞書を選択してください' },
  { re: /^字段 (.+)：默认值 (.*) 不符合字段类型$/, 'en-US': 'Field $1: the default value $2 doesn\'t match the field type', 'ja-JP': 'フィールド $1：デフォルト値 $2 がフィールドの型に合いません' },
  { re: /^模块 (.+) 已存在，请换一个模块名$/, 'en-US': 'Module $1 already exists. Choose another name.', 'ja-JP': 'モジュール $1 はすでに存在します。別の名前にしてください。' },
  { re: /^权限码 (.+) 已存在，请换一个模块名$/, 'en-US': 'Permission code $1 already exists. Choose another module name.', 'ja-JP': '権限コード $1 はすでに存在します。別のモジュール名にしてください。' },
