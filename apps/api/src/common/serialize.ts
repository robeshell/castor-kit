/**
 * 输出格式对齐 Python `datetime.isoformat()`：`YYYY-MM-DDTHH:mm:ss[.ffffff]`，无 `Z`，UTC 值，
 * 微秒为 0 时不带小数部分（变长格式）。
 *
 * 数据库里的 timestamp 由驱动原样返回文本（见 db/client.ts 的类型解析器与 schema 的 mode:'string'），
 * `toIso()` 只做两件事，不经过 JS `Date`（时区与微秒精度问题）：
 * - 日期与时间之间的空格换成 `T`
 * - 小数秒右补 0 到 6 位：PostgreSQL 文本输出会去掉末尾的 0（`.68794`），Python 固定输出 6 位（`.687940`）
...
/**
 * 应用侧生成的当前 UTC 时间，等价 Python `datetime.utcnow().isoformat()`。
 * JS 只有毫秒精度，微秒部分补 000；毫秒为 0 时与 Python 一样省略小数。
 */
...
/** 等价 Python `dt.strftime('%Y-%m-%d %H:%M:%S')`（输入为数据库原样返回的 timestamp 文本），空值返回 '' */
export function formatDateTime(value: string | null | undefined): string {
  return value ? value.replace('T', ' ').slice(0, 19) : ''
}

/** 等价 Python `d.strftime('%Y-%m-%d')`，空值返回 '' */
