/**
 * 手写 5 段 cron 匹配器（分 时 日 月 周），从 AuraStack backend/app/admin/schema/scheduled_task.py 的
 * parse_cron_expression / compute_next_run_at / _parse_cron_field / _parse_num 原样移植。
 *
 * 与标准（Vixie）cron 的差异必须保留（rewrite-plan §5.6）：
...
/** Python `str(int(text))`：去掉前导 0、Unicode 数字转 ASCII（大数也保持精确文本） */
...
/** 墙上时间 → 1970 起的微秒数（用于两个数据库时间求差，等价 Python datetime 相减） */
