...
import { STATIC_LOGS, STATIC_WORK_HISTORY } from '@/modules/component_center/pages/admin/detail_tabs_page/demo-content'

// Avatar colors are business data persisted to the database (backend default #4080FF), not page styling
  ...
  return (
    <div className="space-y-6">
      <dl className="grid gap-x-8 gap-y-5 sm:grid-cols-2">
        <InfoItem icon={Building2} label={t('部门')} value={member.department} />
        <InfoItem icon={Briefcase} label={t('职位')} value={member.role_title} />
        <InfoItem icon={Mail} label={t('邮箱')} value={member.email} />
        <InfoItem icon={Phone} label={t('电话')} value={member.phone} />
        <InfoItem icon={CalendarDays} label={t('入职日期')} value={member.join_date ? member.join_date.slice(0, 10) : ''} />
      </dl>
      {member.bio ? (
        <div className="space-y-1.5">
          <p className="text-muted-foreground text-xs">{t('个人简介')}</p>
    ...
    <ul className="divide-y">
      {STATIC_LOGS.map((log) => (
        <li key={log.time} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
          ...
          <span className="text-muted-foreground min-w-0 flex-1 truncate text-xs">{t('操作人：{{name}}', { name: log.operator })}</span>
        ...
        <Panel padded={false} className="flex flex-col lg:h-[calc(100dvh-210px)] lg:min-h-[480px]" bodyClassName="flex min-h-0 flex-1 flex-col">
          ...
          {!loading && members.length ? (
            <div className="text-muted-foreground border-t px-4 py-2.5 text-xs tabular-nums">
              {search.trim()
                ? t('{{shown}} / {{total}} 位成员', { shown: filteredMembers.length, total: members.length })
              ...
              >
                <div className="flex flex-col gap-4 p-5 pb-0 sm:flex-row sm:items-center">
                  ...
                  <div className="flex shrink-0 gap-2">
                    ...
                    <ConfirmAction
                      title={t('确定删除成员「{{name}}」？', { name: selectedMember.name })}
                      description="此操作不可恢复。"
                      confirmText="删除"
