...
- Visual modeler (System → Configuration, development only, super admins): describe a module or let AI draft it, edit fields with labels, required / unique / default values, fixed options and dictionary fields, preview the form and list live, then generate — code, migration, menu and permissions, API docs and the verify gate run in a detached process with live output, and a failing step undoes everything. Generated modules can be undone. The same spec works from the command line: `pnpm scaffold -- --spec <file>` adds Chinese labels, NOT NULL / UNIQUE / defaults, `enum` and `dict` field types, a field-rules test and the menu under a new 业务管理 group.
