...
- Global AI assistant: a floating button in the bottom-right corner of every page (⌘/Ctrl + J) opens a chat that answers questions, looks data up and — after the user allows each call on a confirmation card showing the method, path and body — changes it. Its tools call the API as the signed-in user, so permissions, data scope, demo limits and operation logs apply; approvals are signed with a key derived from `SECRET_KEY`; account, security, settings and import / export routes are off limits. The assistant finds routes through the registered routes and the bundled OpenAPI document; large results are shrunk by structure (not cut mid-record) and the last step always answers in text. Off by default (`ai.assistant_enabled` in System settings → AI, needs a configured model); API tokens can't call it and demo mode counts it toward the AI quota.
- Scaffold `--spec <file>`: a JSON module spec — what AI agents write after inferring a feature — gives Chinese titles and labels, required / unique / default values (NOT NULL, UNIQUE, column defaults, checks in the generated service), `enum` fields with fixed options and `dict` fields backed by the data dictionary (`useDictOptions`), a generated field-rules test, and the menu with button permissions added to `seed-rbac.ts` under a new 业务管理 group with names in the menu locales. The spec is validated before anything is written.
...
- `pnpm scaffold` writes the new module's eight endpoints into `docs/apifox-full.openapi.json` — Chinese summaries, permissions, data scope, field types, required / unique / default values, options, dictionaries, response shapes, API-token security — so a scaffolded module passes the OpenAPI check (test gate and `pnpm verify`) without hand-written docs. A module that is already documented is left alone, like the generated code files; `--dry-run` only reports it.
- The OpenAPI document (`docs/apifox-full.openapi.json`) now documents every registered API operation completely — Chinese summary, description with the required permission, one tag and Apifox folder, path and query parameters, request body fields, response structure and error codes, and API-token security where tokens are accepted — written from the code; 102 skeleton entries whose uppercase method keys tools ignored, 51 legacy `{int:x}` keys and 4 undocumented operations are gone. AGENTS.md gains an "OpenAPI 编写规范" section, enforced by a new API test and by `pnpm verify` (`openapi_sync` now blocks); `pnpm openapi:generate` adds per-method stubs with lowercase methods and path parameters and `--strict` lists every operation that breaks the rules. The AI assistant's API catalog now shows allowed values and short descriptions of body fields.
