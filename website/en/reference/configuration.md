...
| `LOGIN_MAX_FAILURES` | Maximum failed login attempts (counted separately per IP and per username) | `10` |
...
| `AI_SQL_DATABASE_URL` | Read-only connection used by AI Data Query; should point at a non-superuser read-only account | Falls back to the main database connection in development / test (still forced read-only); **required in production** |
...
Set in the `Dockerfile`; you normally don't need to change them:
