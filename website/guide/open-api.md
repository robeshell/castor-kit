...
下列账号与安全类接口一律不接受 Token，即使 Token 拥有全部权限：登录 / 退出、找回密码、修改密码、验证身份、两步验证（包括管理员重置别人的两步验证）、个人设置、在线用户与会话、API Token 管理本身、修改系统设置与测试按钮、Webhook 的新增 / 修改 / 删除和查看密钥。读取系统设置、Webhook 列表这类只读接口可以用。
