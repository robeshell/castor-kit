...
`--spec` 读取一个 JSON 文件，[在线建模](#在线建模) 页面生成模块时用的也是它：
...
「系统管理 → 系统配置 → 在线建模」把上面的流程搬进浏览器：填写模块名、标题和字段（也可以用一句话描述，让 AI 起草字段清单），右侧实时预览新增表单和列表，确认后点「生成模块」。页面依次执行：

1. 生成代码（`scaffold --spec`，含菜单与按钮权限）
2. 迁移数据库（`db:migrate`）
3. 同步菜单权限（`seed:rbac --incremental`）
4. 更新接口文档（`openapi:generate`）
5. 门禁检查（`verify --module <name> --skip-build`）

每一步的输出实时显示在页面上。任何一步失败都会撤销本次生成的内容：删掉生成的文件、从注册文件和 `seed-rbac.ts` 里去掉这个模块、删除开发库和测试库里的表、迁移记录与菜单。成功后菜单立即出现，生成的代码就在仓库里，可以继续手动修改后提交。

- **只在开发环境可用**（`NODE_ENV=development`，即 `pnpm dev`），且只对超级管理员开放；生成和撤销要求近期验证过身份，API Token 不能调用。生产环境不注册这些接口，页面只显示说明
- 生成过程在一个独立进程里运行（`apps/api/scripts/modeler-run.ts`）：后端因为新代码重启时它不受影响，页面会在重启后自动接着显示进度。任务状态和日志保存在 `apps/api/.modeler/`（不进 git）
- 「已生成的模块」里可以撤销建模器生成的模块，删除它的代码、菜单与权限、数据表（包括其中的数据）和迁移记录。只有当它的迁移是最后一个时才能撤销，之后又有其他迁移时需要手动处理
- 一次只能运行一个任务
- AI 起草与翻译使用「系统设置 → AI」里配置的模型；没配置时这两个按钮不显示，手动填写即可
