...
| ページ | ルート | 説明 |
|---|---|---|
| ユーザー管理 | `/system/users` | ユーザーの追加・編集・削除、ロールの割り当て、インポート / エクスポート（標準的な一覧ページの参考実装） |
| ロール権限 | `/system/roles` | ロールの管理とメニュー / ボタンの権限付与 |
...
このほかに、ホーム（`/dashboard`）と個人設定ページ（`/profile`）があります。
