...
`--spec` は JSON ファイルを読み込みます。[オンラインモデリング](#オンラインモデリング) がモジュールを生成するときも同じファイルを使います。
...
「システム管理 → システム構成 → オンラインモデリング」は上の流れをブラウザで行います。モジュール名、タイトル、フィールドを入力し（一文で機能を説明して AI にフィールドを作成させることもできます）、右側で追加フォームと一覧のプレビューを確認してから「モジュールを生成」をクリックします。ページは次の手順を順に実行します。

1. コード生成（`scaffold --spec`、メニューとボタン権限を含む）
2. データベースのマイグレーション（`db:migrate`）
3. メニューと権限の同期（`seed:rbac --incremental`）
4. API ドキュメントの更新（`openapi:generate`）
5. ゲートチェック（`verify --module <name> --skip-build`）

各手順の出力はリアルタイムで表示されます。いずれかの手順が失敗すると、今回の生成内容はすべて取り消されます。生成したファイルを削除し、登録ファイルと `seed-rbac.ts` からモジュールを除き、開発用・テスト用データベースからテーブル、マイグレーション記録、メニューを削除します。成功するとメニューがすぐに表示され、生成したコードはリポジトリにあるので、そのまま手動で修正してコミットできます。

- **開発環境でのみ使用できます**（`NODE_ENV=development`、つまり `pnpm dev`）。スーパー管理者のみが使え、生成と取り消しには直近の本人確認が必要で、API トークンでは呼び出せません。本番環境ではこれらの API を登録せず、ページには説明だけを表示します
- 生成は別プロセス（`apps/api/scripts/modeler-run.ts`）で実行されるため、新しいコードの読み込みでバックエンドが再起動しても影響を受けません。再起動後、ページは自動で進行状況の表示を再開します。ジョブの状態とログは `apps/api/.modeler/`（git 管理外）に保存されます
- 「生成済みモジュール」から、モデラーで生成したモジュールを取り消せます。コード、メニューと権限、テーブル（データを含む）、マイグレーション記録が削除されます。そのマイグレーションが最新の場合のみ取り消せます。後から別のマイグレーションがある場合は手動で対応してください
- 一度に実行できるジョブは 1 つです
- AI による下書きと翻訳は「システム設定 → AI」で設定したモデルを使います。未設定の場合はボタンが表示されないので、手動で入力してください
