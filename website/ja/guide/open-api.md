...
- エンドポイントとパラメーターはリポジトリの `docs/apifox-full.openapi.json` にあります（セキュリティスキーム `bearerAuth`）
...
次のアカウント・セキュリティ系エンドポイントは、すべての権限を持つトークンでも受け付けません：ログイン / ログアウト、パスワード再設定、パスワード変更、本人確認、2 段階認証（管理者による他人のリセットを含む）、個人設定、オンラインユーザーとセッション、API トークン管理そのもの、システム設定の変更とテストボタン、Webhook の追加・変更・削除とシークレットの表示。システム設定や Webhook 一覧の読み取りなど、読み取り専用のエンドポイントは使えます。
