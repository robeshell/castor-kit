...
- 保存とテストには 10 分以内の本人確認が必要で、保存のたびにすべてのスーパー管理者へ通知されます。詳しくは[システム設定の保護](/ja/guide/security#settings-protection)を参照してください。本番環境では少なくとも `APP_BASE_URL` と `SMTP_HOST` を環境変数で固定することをおすすめします
