...
| 3 | [在线可视化建模与 AI 助手](#6-在线可视化建模与-ai-助手) | 中 | 1、2 | 已完成 |
| 3 | [审批流](#7-审批流) | 低 | 1 | 未开始 |
...
- **开关**：系统设置「允许使用 API Token」（`security.api_tokens_enabled`），默认关闭；关闭后已有 token 全部停用（保留，重新打开恢复）；演示模式不可开启
...
**实现补充**（迁移 `0007_open_api`）：

- 创建 token、新增 / 编辑 Webhook、查看 / 重新生成密钥都要求近期验证身份（与系统设置相同的 10 分钟窗口）；每人最多 20 个有效 token
- 新增 Webhook、修改 Webhook 地址时通知所有超级管理员
- 投递卡在「发送中」超过 5 分钟（进程中途退出）会被重新认领；重试任务每 30 秒运行一次（调度器内置任务 `webhook-retry`）
- `seed-rbac` 按实际 ID 解析父菜单，固定 ID 被占用的旧库也能增量同步
...
- 不走 WebSocket，改为页面轮询任务进度（带日志偏移）：生成代码会让 dev server 重启，轮询在重启后自然恢复，WebSocket 需要额外的重连与补发
- 生成在独立进程（`scripts/modeler-run.ts`）里运行，步骤比原计划多了「更新接口文档」（OpenAPI 覆盖率测试要求新路由有文档）；失败时撤销的范围包括数据库中的表、迁移记录与菜单
- 新增「撤销已生成的模块」（仅限建模器生成、且迁移是最后一个的模块）
- 除必填 / 唯一 / 默认值外，spec 还支持中文标签、固定选项（`enum`）、数据字典（`dict`）、多语言译文和菜单登记；命令行用 `pnpm scaffold -- --spec <文件>`，与页面生成的代码一致
- 生成的模块挂在新建的顶级目录「业务管理」（ID 1000）下
...
| 2026-09-26 | 移除「5. 第三方登录（OAuth / OIDC）」：暂不规划；其余章节编号不变 |
| 2026-09-26 | 系统设置扩展为配置中心：邮件、文件存储、上传限制、AI 模型、网站地址、登录锁定改在页面上配置（密钥加密存储、可测试），环境变量改为可选的锁定手段；只有启动前需要的配置留在环境变量。保存 / 测试需要近期验证身份，变更通知超级管理员，地址类设置有 SSRF 防护（迁移 `0006_session_verified_at`）。系统管理菜单分为四组 |
...
| 2026-09-26 | 完成「6. 在线可视化建模与 AI 助手」：`scaffold --spec`（中文标签、必填、唯一、默认值、固定选项、数据字典、菜单登记）、「在线建模」页面（开发环境）、生成失败自动撤销与手动撤销；实现补充见该节 |
