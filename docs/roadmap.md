...
| 3 | [在线可视化建模与 AI 助手](#6-在线可视化建模与-ai-助手) | — | — | 放弃（保留其中的 `scaffold --spec` 与已完成的 AI SDK 迁移） |
| 3 | [审批流](#7-审批流) | 低 | 1 | 未开始 |
...
> **已放弃**（2026-09-26）：做出了可用的第一版后判断，要覆盖真实业务就得继续补表间关联、主从表、业务规则、修改已有模块，会演变成低代码平台，与 castor-kit「让 AI agent 生成功能代码」的方向重复。保留两部分：`pnpm scaffold -- --spec`（agent 把推断出的规格写成 JSON 再生成，中文标签、必填 / 唯一 / 默认值、固定选项、数据字典、菜单一次到位），以及已完成的 AI 调用层迁移（Vercel AI SDK）。以下为原计划，仅供参考。
