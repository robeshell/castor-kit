...
### 4.17 在线建模（开发环境）
- `modules/admin/modeler`：只有 `config.modelerEnabled`（即 `NODE_ENV=development`）时注册路由，仅超级管理员，生成 / 撤销要求近期验证身份，`API_TOKEN_DENIED` 拒绝 token；`app-info` 的 `modeler` 告诉前端是否可用。生成器（`scripts/scaffold.ts`）在运行时按文件路径动态加载，不进生产包。
- 生成：页面把 spec（与 `pnpm scaffold -- --spec` 的文件相同）交给 `POST /jobs`，服务端写 `apps/api/.modeler/jobs/<id>/{spec,state}.json` 后以 detached 子进程启动 `scripts/modeler-run.ts`（stdout 指向 `log.txt`，`unref()`）：dev server 因生成的文件变化而重启时它不受影响。步骤：`scaffoldFromSpec`（`onChange` 记录新建的文件，迁移按 journal 最后一项的变化识别）→ `db:migrate` → `seed:rbac --incremental` → `openapi:generate` → `verify --module --skip-build`。生成完成第一步就把模块写进 `.modeler/modules.json`，任何一步失败都按它撤销。`lock` 文件保证一次一个任务（进程已不在时视为无锁）。
- 撤销（`scripts/lib/modeler-undo.ts`）逐行编辑共享文件而不是恢复备份：`db/schema/index.ts` 的 export、`router.ts` 的 import 与注册、`seed-rbac.ts` 的菜单行（「业务管理」目录没有子菜单时一并删除）、菜单译文、OpenAPI 路径、drizzle journal（仅当该迁移是最后一项，否则拒绝，保护迁移链），再删生成的文件；开发库与测试库里 `DROP TABLE`、删 `drizzle.__drizzle_migrations`（按 journal 的 `when`）、`role_menus` 与菜单。
- 前端（`pages/modeler`）每 800 ms 轮询 `GET /jobs/:id?offset=` 增量读日志，请求失败（后端重启中）静默重试；Vite 因新页面文件整页刷新后，页面按 `meta.running` 继续跟踪任务。`AuthContext` 首次加载用户时后端不可达会重试约 30 秒，只有 401 才当作未登录。
- 生成的菜单挂在「业务管理」（`biz`，ID 1000）下，模块 ID 1001–1999；表单页、校验与规则测试由 spec 的 label / required / unique / default / options / dict 生成（见 `scripts/scaffold.ts` 文件头）。AI 起草（`generateText` + `Output.object`）把 JSON 结构写进提示词：OpenAI 兼容接口只收到 `response_format: json_object`，收不到 schema。
