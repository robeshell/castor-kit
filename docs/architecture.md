...
### 4.17 AI 小助手
- `POST /api/admin/assistant/chat`（`modules/admin/assistant`）：只需登录；开关 `ai.assistant_enabled` 默认关闭、没有配置模型时不能打开，关闭时 403「AI 小助手未开启」，`app-info.assistant` 告诉前端是否显示。请求体 `{ messages: UIMessage[], context?: { path, title } }`，消息校验与 AI 对话共用 `parseChatMessages`；响应同 §4.10（UI message stream，`reply.send(Readable.fromWeb(...))`）。API Token 不能调用（`API_TOKEN_DENIED`）；演示模式计入 AI 额度（`DEMO_AI_PATHS`）。
...
- 受保护的写操作直接拒绝、不请用户确认：`needsApproval` 是函数，先跑 `refusal()`（路径不在目录 / 不开放 → 400；当前用户自己的账号、超级管理员账号、超级管理员角色、请求体 `role_ids` 含超级管理员 → 403），被拒的调用记下 toolCallId，`execute` 对它们直接返回拒绝结果，已批准的调用执行前也再查一次。
