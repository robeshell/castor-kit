...
- **CI**（`.github/workflows/ci.yml`）：`pnpm install` → lint → typecheck → 空库 `setup-once` → api vitest（pg service）→ `pnpm verify --skip-build` → web 单测 → `vite build`。`deploy.yml` 执行 `git pull && docker compose up -d --build`；`docs.yml` 构建文档站。
